...
class MyApp extends StatelessWidget { const MyApp({super.key});
  @override Widget build(BuildContext context) {
    return MaterialApp(debugShowCheckedModeBanner: false, theme: ThemeData(scaffoldBackgroundColor: kBg, appBarTheme: const AppBarTheme(backgroundColor: kCard, foregroundColor: Colors.white)),
      home: Scaffold(appBar: AppBar(title: const Text('Download URLs')),
        body: const Padding(padding: EdgeInsets.all(16), child: Center(child: Text('Get URL:\n\nfinal ref = FirebaseStorage.instance\n  .ref("avatars/user123.png");\nfinal url = await ref.getDownloadURL();\n\nUse with Image.network(url)', style: TextStyle(color: Colors.white, fontSize: 16, fontFamily: 'monospace'))))));
  }
}
