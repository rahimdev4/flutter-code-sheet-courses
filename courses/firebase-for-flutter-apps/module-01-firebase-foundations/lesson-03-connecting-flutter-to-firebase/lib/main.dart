...
Future<void> main() async {
  ...
  await Firebase.initializeApp(); // Requires firebase_options.dart from FlutterFire CLI
  runApp(const MyApp());
}
  ...
  @override Widget build(BuildContext context) {
    return MaterialApp(debugShowCheckedModeBanner: false, theme: ThemeData(scaffoldBackgroundColor: kBg, appBarTheme: const AppBarTheme(backgroundColor: kCard, foregroundColor: Colors.white)),
      home: Scaffold(appBar: AppBar(title: const Text('Connected!')),
        body: const Center(child: Text('Firebase is initialized!', style: TextStyle(color: kAccent, fontSize: 24)))));
  }
}
